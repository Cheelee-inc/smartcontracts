...
contract(NFTGlassesConfig.contractName, () => {
  ...
  describe("Token Rate Limit", async () => {
    it("Adding Token limit", async function () {
      ...
      expect(resultWaited.events[0].args.index).to.equal("1");
      expect(resultWaited.events[0].args.newLimit).to.equal("5");
    });
    ...
    it("Testing Day and Month limits", async function () {
      ...
      const signature2 = await signatureSigner._signTypedData(domain, TrNftSig.Pass, {
        nonce: 14,
        id: 11,
        address_to: deployer.address,
        ttl: timestamp,
      ...
      assert.equal(
        String(await commonBlacklist.getUserTokenDayTransfers(nftGlasses.address, deployer.address)),
        "1"
      );
      ...
      await expectRevert(
        nftGlasses.connect(deployer).transferFrom(
          deployer.address,
          treasury.address,
          11
        ),
        "NFT: Spender has reached the day limit"
      );
      ...
      assert.equal(
        String(await commonBlacklist.getUserTokenMonthTransfers(nftGlasses.address, deployer.address)),
        "3"
      );
    });

    it("Adding treasury to exclusion list", async function () {
      const domain = TrNftSig.eip712Domain(treasury.address, (await ethers.provider.getNetwork()).chainId)
      const timestamp = await currentTimestamp() + 1000
      const signatureSigner = await ethers.getImpersonatedSigner(deployer.address)
      const signature = await signatureSigner._signTypedData(domain, TrNftSig.Pass, {
      ...
      expect(resultWaited.events[0].args.token).to.equal(nftGlasses.address);
      expect(resultWaited.events[0].args.dayLimit).to.equal("1");
      expect(resultWaited.events[0].args.monthLimit).to.equal("1");

      result = await commonBlacklist.connect(moderator).addContractToExclusionList(
        treasury.address
      );

      resultWaited = await result.wait();

      expect(resultWaited.events[0].args.token).to.equal(treasury.address);
      ...
      await expectRevert(
        ...
        "NFT: Spender has reached the month limit"
      );
    });
  });
  ...
  describe("Tokens disabling", async () => {
    it("Testing disabling NFT", async function () {
      ...
      expect(resultWaited.events[0].args.token).to.equal(nftGlasses.address);
      expect(resultWaited.events[0].args.dayLimit).to.equal("2");
      expect(resultWaited.events[0].args.monthLimit).to.equal("4");
