...
async function main() {
    ...
    // TODO: set N
    for (let i = 0; i < 16979; i += chunkSize) {
        ...
        let results = await Promise.all(chunk.map(
            (recipient, chunkIdx) => sendTx(owner, i + chunkIdx, recipient as string, initTxCounter + i + chunkIdx)
        ));
...
async function sendTx(owner: SignerWithAddress, i: number, to: string, nonce: number): Promise<boolean> {
